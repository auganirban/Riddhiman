...
%U = dlmread('SVM_Train_Not_Converging.txt');
...
s2 = scatter(U(:,1),U(:,2),sz,'MarkerEdgeColor',[1 0 0],...
              ...
              'LineWidth',1);
s2.Marker = 'o';
legend('Positive Class','Negative Class','Location','northwest');
title('Positive and Negative classes for classification');
...
c = cvpartition((size1+size2),'KFold',30);
...
% 
% legend('Positive','Negative','Support Vector','Location','Northwest')
% hold off
%% K Fold Loss
...
print('Optimized_Classifier_Exp6','-dpdf','-bestfit');
axis equal
hold off
...
[labels, postprob] = predict(ScoreSVMModel,data); %for predicting posterior prob of data points

% table(theclass,label,scores,PostProbs,'VariableNames',...
